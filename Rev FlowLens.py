...
6. 可视化：
   ...
   - 202602新增 Purchases：Paid vs Non-paid；Paid 内 Ad Channels
7. 20260304新增 funnel（只在 Paid 内）：
   - Paid funnel 总览：lower / middle / high / No funnel
   - Paid 渠道 × funnel：各渠道内部 funnel 构成
8. 20260304新增 Actual Revenue 输入框
...
def find_header_row(lines: list[str], header_key: str = "Session default channel group") -> int:
    """
    找到真正字段名那一行（包含空行的原始行号）
    后面 read_csv 会设置 skip_blank_lines=False，确保行号一致
    """
...
def allocate_paid_attribution(paid_df: pd.DataFrame, metric_col: str, paid_keywords: list[str]) -> pd.DataFrame:
    ...
    - funnel 分摊绑定渠道：source1->funnel1；source2->funnel2
    """
    rows = []
...
if uploaded_file is not None:
    try:
        ...
        # ----------------------------------------------------
        # 3) 判断是否含 funnel 两列 -> 决定 core_cols
        # ----------------------------------------------------
        has_funnel_cols = (
        ...
        # ----------------------------------------------------
        # ✅ 4) 修复 GA4 导出首列空表头/Unnamed 导致“整体右移错位”
        # ----------------------------------------------------
        need_shift = False
        ...
        # 如果列名看起来正常，但错位特征明显：Session default channel group 列大量是 "google / cpc" 这种
        ...
        # ----------------------------------------------------
        # 10) ✅ Actual Revenue 输入框 -> Adjusted revenue
        # ----------------------------------------------------
